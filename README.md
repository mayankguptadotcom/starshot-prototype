...
#### Which database?
By default, Starshot sets up a SQLite database, so that you don't need to do any extra setup to get started.

But if you want to install into another database, you can use the following commands instead of the one above, substituting the real database URL instead of `mysql://drupal:drupal@localhost/starshot`

```
composer create-project drupal/starshot-project --repository='{"type":"vcs","url":"https://github.com/phenaproxima/starshot-prototype"}' --stability=dev --no-install
cd starshot-project
composer install
